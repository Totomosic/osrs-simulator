...
const selectedLineOfSightSourceActor = computed(() => {
    if (snapshot.value === null) {
        return null;
    }

    if (snapshot.value.player.id === selectedLineOfSightSourceActorId.value) {
        return snapshot.value.player;
    }

    return (
        snapshot.value.npcs.find(
            (npc) => npc.id === selectedLineOfSightSourceActorId.value,
        ) ?? null
    );
});
const lineOfSightRange = computed(
    () => selectedLineOfSightSourceActor.value?.weapon.range ?? 1,
);
const lineOfSightTiles = computed<DebugTile[]>(() => {
    const scenarioInstance = scenario.value;
    const sourceActor = selectedLineOfSightSourceActor.value;

    if (scenarioInstance === null || sourceActor === null) {
        return [];
    }
...
const actorLineOfSightDetails = computed(() => {
    ...
    return {
        playerToNpc: scenario.value.hasActorLineOfSight(
            ...
            snapshot.value.player.weapon.range,
        ),
